...
app.component('answer-input', {
    ...
    data: function () {
        return {
            /* 送信ボタン上下に表示されるメッセージ */
            okMessage: 'パスワードの一致を確認しました',
            ngMessage: 'パスワードが一致しません',
            message: '',
            inputAnswer: '',
        }
    },
    ...
    <div class="answer__container">
      <div class="answer">
        <input type="text" v-model="inputAnswer" placeholder="Enter the code">
      </div>
      <p v-if="message === ngMessage" class="err-message">{{ message }}</p>
      <button v-on:click="judgement(inputAnswer)"><b><font color="#000">OK</button></b>
    ...
    methods: {
        judgement(answer) {
            ...
            } else if (answer == "delete") { //入力値がゲームオーバーキーワードの場合(編集点)
                window.location.href = "over.html"; //ここに入れる
            } else { // 一致しない場合
